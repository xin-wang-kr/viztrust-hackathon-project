...
df_trust = pd.read_csv('trust_evaluation/trust_evaluation.csv')
# row = df_trust.iloc[0,:4] 
# trust_data = pd.Series(row.to_list(), index=row.index)

# load user engagement data
df_engagement = pd.read_csv('trust_evaluation/user_engagement.csv')

# load politeness strategies data
df_politeness_strategies = pd.read_csv('trust_evaluation/politeness_strategies.csv')

# load user prompt emotion data
df_emotion = pd.read_csv('trust_evaluation/user_prompt_emotion.csv')
...
df_trust.loc[:,["competence_trust", "benevolence_trust", "integrity_trust", "predictability_trust"]] = df_trust.loc[:,["competence_trust", "benevolence_trust", "integrity_trust", "predictability_trust"]].apply(lambda x: (x-0)/ (7-0), axis=0)
...
with col1:
    ...
    # Add traces
    fig_1.add_trace(go.Scatter(x=df_trust['n_turn'].astype(str), y=df_trust['competence_trust'], name='Competence Trust', 
    ...
    fig_1.add_trace(go.Scatter(x=df_trust['n_turn'].astype(str), y=df_trust['benevolence_trust'], name='Benevolence Trust', 
                               line_color="#fbc02d", mode='lines+markers', opacity=0.8, marker=dict(opacity=1)))
    fig_1.add_trace(go.Scatter(x=df_trust['n_turn'].astype(str), y=df_trust['integrity_trust'], name='Integrity Trust', 
                               line_color='#66bb6a', mode='lines+markers', opacity=0.8, marker=dict(opacity=1)))
    fig_1.add_trace(go.Scatter(x=df_trust['n_turn'].astype(str), y=df_trust['predictability_trust'], name='Predictability Trust', 
